 ...
 */
package com.bai.android.data.extension.avatar;
...
import com.bai.android.data.OnLowMemoryListener;
import com.bai.android.data.SettingsManager;
import com.bai.android.data.account.AccountItem;
import com.bai.android.data.account.OAuthManager;
...
import com.bai.xmpp.avatar.VCardUpdate;
...
OnPacketListener {
	...
	public Drawable getAccountAvatar(String account) {
		...
		if (value != null) {
			...
			return application.getResources().getDrawable(accountAvatarSet.getResourceId(account));
		}
	}

	public Bitmap getAccountAvatarBitmap(String account) {
		String jid = OAuthManager.getInstance().getAssignedJid(account);
		if (jid == null)
			jid = account;
		Bitmap value = getBitmap(Jid.getBareAddress(jid));
		if (value != null)
			...
			return BitmapFactory.decodeResource(application.getResources(), accountAvatarSet.getResourceId(account));
	}
	...
	public Bitmap getUserBitmap(String user) {
		...
		if (value != null) {
			...
			return BitmapFactory.decodeResource(application.getResources(), userAvatarSet.getResourceId(user));
		}
	}
