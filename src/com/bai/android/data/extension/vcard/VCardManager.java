...
import com.bai.android.data.connection.OnAuthorizedListener;
...
import com.bai.android.data.roster.OnRosterChangedListener;
...
import com.bai.android.data.roster.StructuredName;
 ...
 * 
 */
public class VCardManager implements OnLoadListener, OnAuthorizedListener, OnPacketListener, OnDisconnectListener, OnRosterReceivedListener, OnAccountRemovedListener {
	...
	// for callbacks, eg. vcard update
	private String expectedPacketID;
	private VcardUpdateCallback vcardUpdateCallback;

	private VCard vcardToUpdate;
	...
	private void assignValueForUpdatingUserProfile(String account, String username, VCard vcardToUpdate) {
		this.account = account;
		this.username = username;
		this.vcardToUpdate = vcardToUpdate;
	}
	
	public void set(String account, String username, VCard vcardToUpdate) {
		
		if (!connected) {
			updatePending = true;
			assignValueForUpdatingUserProfile(account, username, vcardToUpdate);
			return;
		}

		updatePending = false;
		
		VCard packet = vcardToUpdate;
//		packet.setTo(bareAddress);
	...
	public void onPacket(ConnectionItem connection, final String bareAddress,
		...
		if (packet instanceof Presence
			...
			IQ iq = (IQ) packet;
			if (vcardUpdateCallback != null && expectedPacketID != null && expectedPacketID.equals(packet.getPacketID())) {
				sendVcardCallbackResult(iq);
			}
	...
	/** return result to registered callback and clear it */
	private void sendVcardCallbackResult(IQ iq) {
		if (iq.getType() == Type.ERROR) {
			vcardUpdateCallback.onCallback(iq.getError().toString(), false);
		}
		else {
			vcardUpdateCallback.onCallback("Success.", true);
		}
		vcardUpdateCallback = null;
	...
	public void onAuthorized(ConnectionItem connection) {
		if (!(connection instanceof AccountItem))
			return;
		connected = true;
		if (updatePending) set(account, username, vcardToUpdate);
	}

	public void registerUpdateCallback(VcardUpdateCallback vcardUpdateCallback, String packetId) {
		this.vcardUpdateCallback = vcardUpdateCallback;
		expectedPacketID = packetId;
	}

	/** 
	 * Broadcasts that presence information with avatar to all contacts who are subscribed to the user's presence information
	 * @param account
	 * @param avatarHash
	 */
	public void includeAvatarHashInPresenceBroadcast(String account, String avatarHash) {
		...
		presence.setPhotoHash(avatarHash);
		presence.setPriority(5);

		try {
			ConnectionManager.getInstance().sendPacket(account, presence);
		} catch (NetworkException e) {
			e.printStackTrace();
		}
	}
