...
import com.bai.xmpp.muc.Affiliation;
import com.bai.xmpp.muc.MUC;
import com.bai.xmpp.muc.Role;
...
public class RoomChat extends AbstractChat {
	...
	protected boolean notifyAboutMessage() {
		if (this.getRoom().equals(MUCManager.PersistentMUCChats.GLOBAL)) return false;
		if (this.getRoom().equals(MUCManager.PersistentMUCChats.RECOMMENDED)) return false;

		if (SettingsManager.eventsMessage() == SettingsManager.EventsMessage.chat || SettingsManager.eventsMessage() == SettingsManager.EventsMessage.chatAndMuc) {
//			Date now = new Date();
//			if (this.getLastTime()!= null) {
//				int diffInDays = (int)( (now.getTime() - this.getLastTime().getTime()) / (86400000)); // 1000 * 60 * 60 * 24 = 86400000 (millis in one day)
...
//			}
	...
	private void onAvailable(String resource) {
		if (isSelf(resource)) {
			...
			if (isRequested()) {
				if (showStatusChange())
					//					newMessage(	DON'T DISPLAY - "YOU HAVE JOINED CHAT AS ..."
					//							resource,
				...
				//				if (showStatusChange())	DON'T DISPLAY - "YOU HAVE JOINED CHAT AS ..."
				//					newAction(resource, null, ChatAction.complete);
			}
		} else {
			//			if (state == RoomState.available)	DOSNT DISPLAY "YOU JOINED CHAT AS .... "
			//				if (showStatusChange()) newAction(resource, null, ChatAction.join);
		}
		if (invitationCallbackTarget != null) invitationCallbackTarget.invitationCallback();
	}
	...
	private InvitationCallback invitationCallbackTarget;

	public void registerInvitationCallback(InvitationCallback callbackTarget) {
		this.invitationCallbackTarget = callbackTarget;
	}
}