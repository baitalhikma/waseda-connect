...
import com.bai.android.data.OnCloseListener;
import com.bai.android.data.OnInitializedListener;
...
import com.bai.android.data.extension.muc.RoomInvite;
...
import com.bai.android.ui.ClearNotifications;
import com.bai.android.ui.ContactList;
import com.bai.android.ui.OtherActivity;
import com.bai.android.ui.ReconnectionActivity;
...
public class NotificationManager implements OnInitializedListener,
	...
	public <T extends NotificationItem> void updateNotifications(
		...
		if (!iterator.hasNext()) {
			...
			if (notify == null) {
				...
				top = notify;
				ticker = top.getTitle();
				if (top instanceof RoomInvite) {
					MUCManager.getInstance().setPendingIncomingInvitation(true);
				}
			}
	...
	private void updateMessageNotification(MessageItem ticker) {
		...
		if (messageNotifications.isEmpty()) {
			...
			if (message.getUser().equals(MUCManager.PersistentMUCChats.NOTICE)) {
				chatIntent = new Intent(application.getApplicationContext(), OtherActivity.class);
				chatIntent.putExtra("targetView", "notice");
			} else {
				chatIntent = ChatViewer.createClearTopIntent(application, message.getAccount(), message.getUser());
			}
