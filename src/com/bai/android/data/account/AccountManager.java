...
import com.bai.android.data.OnWipeListener;
import com.bai.android.data.SettingsManager;
import com.bai.android.data.connection.ConnectionState;
import com.bai.android.data.connection.ProxyType;
import com.bai.android.data.connection.TLSMode;
...
//			if (Application.SDK_INT < 8 && protocol == AccountProtocol.wlm) {
			if (Application.SDK_INT < 8) {
				values.recycle();
				continue;
			}
			ArrayList<String> servers = new ArrayList<String>();
			servers.add(values.getString(8));
			...
			accountTypes.add(new AccountType(id, protocol, values.getString(1),
					values.getString(2), values.getString(3), values.getBoolean(4, false),
					values.getString(5), values.getInt(6, 5222), values
							.getBoolean(7, false), servers));
			values.recycle();
		}
		types.recycle();
		away = false;
...
//		String serverName = accountType.getServers().get(0);
