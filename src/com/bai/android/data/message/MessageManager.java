 ...
 */
package com.bai.android.data.message;
...
import com.bai.android.data.SettingsManager.ChatsShowStatusChange;
import com.bai.android.data.account.AccountItem;
import com.bai.android.data.account.AccountManager;
import com.bai.android.data.account.ArchiveMode;
import com.bai.android.data.account.OnAccountArchiveModeChangedListener;
import com.bai.android.data.account.OnAccountRemovedListener;
import com.bai.android.data.account.StatusMode;
import com.bai.android.data.connection.ConnectionItem;
import com.bai.android.data.connection.OnDisconnectListener;
...
import com.bai.android.data.roster.OnRosterReceivedListener;
import com.bai.android.data.roster.OnStatusChangeListener;
...
import com.bai.xmpp.delay.Delay;
...
OnStatusChangeListener {
	...
	public Collection<AbstractChat> getActiveChats() {
		...
		for (AbstractChat chat : chats.values())
			// exclude the system notices
			if (chat.isActive() && !chat.getUser().equalsIgnoreCase(MUCManager.PersistentMUCChats.NOTICE)) {
				collection.add(chat);
			}
	...
	/**
	 * @return Collection with active chats, excluding the global mucs
	 */
	public Collection<AbstractChat> getPersonalActiveChats() {
		Collection<AbstractChat> collection = new ArrayList<AbstractChat>();
		for (AbstractChat chat : chats.values())
			// exclude the system notices
			if (chat.isActive() && !chat.getUser().equalsIgnoreCase(MUCManager.PersistentMUCChats.NOTICE)
					&& !chat.getUser().equalsIgnoreCase(MUCManager.PersistentMUCChats.RECOMMENDED) 
					&& !chat.getUser().equalsIgnoreCase(MUCManager.PersistentMUCChats.GLOBAL)) {
				collection.add(chat);
			}
		return Collections.unmodifiableCollection(collection);
	}
	...
	/**
	 * @return Collection with all chats.
	 */
	public Collection<AbstractChat> getAllChats() {
		Collection<AbstractChat> collection = new ArrayList<AbstractChat>();
		Collection<AbstractChat> allChats = MessageTable.getInstance().getChatHistory(50); //TODO magic number 50 - limit of max chats to display, move to constants
		Date now = new Date();
		for (AbstractChat chat : allChats) {
			if (chat.getLastTime()!= null) {
				int diffInDays = (int)( (now.getTime() - chat.getLastTime().getTime()) / (86400000)); // 1000 * 60 * 60 * 24 = 86400000 (millis in one day)
				// exclude the system notices and messages older than 30 days
				if (diffInDays <= 30 && !chat.getUser().equalsIgnoreCase(MUCManager.PersistentMUCChats.NOTICE)) { //TODO magic number 30 - move to constants/ preferences
					collection.add(chat);
				}
			} else if (!chat.getUser().equalsIgnoreCase(MUCManager.PersistentMUCChats.NOTICE)) { 
				collection.add(chat);
			}
		}
		return Collections.unmodifiableCollection(collection);
	...
	/**
	 * Returns messages and actions for the chat.
	 * @param account
	 * @param user
	...
	/**
	 * Returns only the messages and no actions.
	 * @param account
	 * @param user
	 * @return List of messages or empty list.
	 */
	public Collection<MessageItem> getNonActionMessages(String account, String user) {
		AbstractChat chat = getChat(account, user);
		if (chat == null)
			return Collections.emptyList();
		return chat.getNonActionMessages();
	}
	...
	/** Should a notification be displayed when there is a message from notices. */
	private boolean notifyOnNotices;
	...
	public void onChatChanged(final String account, final String user,
		...
		Application.getInstance().runOnUiThread(new Runnable() {
			@Override
			public void run() {
				for (OnChatChangedListener onChatChangedListener : Application.getInstance().getUIListeners(OnChatChangedListener.class)) {
