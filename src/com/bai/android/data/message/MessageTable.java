...
import com.bai.android.data.DatabaseManager;
import com.bai.android.data.entity.AbstractEntityTable;
...
class MessageTable extends AbstractEntityTable {
  ...
  /**
   * Chat history. (Return a max number of the last chats.)
   * @param limit - the maximum number of chats to return
   * @return
   */
  public ArrayList<AbstractChat> getChatHistory(int limit) {
    // holder for the chats
    ArrayList<AbstractChat> results = new ArrayList<AbstractChat>();
    // get all chats grouped distinctly by user
    SQLiteDatabase db = databaseManager.getReadableDatabase();
    Cursor chatsCursor = db.query(true, NAME, new String[]{Fields.ACCOUNT, Fields.USER, Fields.TIMESTAMP}, null, null, Fields.USER, null, Fields.TIMESTAMP, String.valueOf(limit));
    // efficiency - prepare column indices for the fields we care about
    int accountColInd = chatsCursor.getColumnIndex(Fields.ACCOUNT);
    int userColInd = chatsCursor.getColumnIndex(Fields.USER);
    int timestampColInd = chatsCursor.getColumnIndex(Fields.TIMESTAMP);
    // iterate and construct all chats adding them to the holder
    while (chatsCursor.moveToNext()) {
      AbstractChat chat = new RegularChat(chatsCursor.getString(accountColInd), chatsCursor.getString(userColInd));
      chat.lastTime = new Date(chatsCursor.getLong(timestampColInd));
      results.add(chat);
    }
    return results;
  }
