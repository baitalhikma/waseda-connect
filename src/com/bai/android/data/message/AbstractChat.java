...
public abstract class AbstractChat extends BaseEntity {
	 ...
	 */
	private static final String NO_RECORD_TAG = "com.bai.android.data.message.NO_RECORD_TAG";
	...
	Collection<MessageItem> getNonActionMessages() {
		List<MessageItem> nonActionMessages = new ArrayList<MessageItem>();
		for (MessageItem message : messages) {
			if (message.getAction() == null) nonActionMessages.add(message);
		}
		return Collections.unmodifiableCollection(nonActionMessages);
	}
...
private int i = 0;
	...
	protected MessageItem newMessage(String resource, String text,
		...
		if (save && (unencrypted || (!SettingsManager.securityOtrHistory() && OTRManager
		...
		if (user.equals(MUCManager.PersistentMUCChats.NOTICE)) {
			if (delayTimestamp != null && timestamp !=  null) {
				int diffInDays = (int)( (timestamp.getTime() - delayTimestamp.getTime()) / (86400000)); // 1000 * 60 * 60 * 24 = 86400000 (millis in one day)
		...
		if (showTicker) {
			NotificationManager.getInstance().onMessageNotification(messageItem, notify);
		}
