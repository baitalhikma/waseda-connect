...
import com.bai.android.data.account.CommonState;
...
import com.bai.android.data.roster.GroupManager;
...
import com.bai.android.data.roster.ShowOfflineMode;
import com.bai.android.ui.adapter.AccountConfiguration;
import com.bai.android.ui.adapter.AccountToggleAdapter;
import com.bai.android.ui.adapter.ContactListAdapter;
import com.bai.android.ui.adapter.GroupConfiguration;
import com.bai.android.ui.dialog.AccountChooseDialogBuilder;
...
OnItemClickListener, OnLongClickListener, TextWatcher, OnLoadListener, OnVCardListener {

  /**
   * Select contact to be invited to the room was requested.
   */
  private static final String ACTION_ROOM_INVITE = "com.bai.android.ui.ContactList.ACTION_ROOM_INVITE";

  private static final long CLOSE_ACTIVITY_AFTER_DELAY = 300;

  private static final String SAVED_ACTION = "com.bai.android.ui.ContactList.SAVED_ACTION";
  private static final String SAVED_VIEW = "com.bai.android.ui.ContactList.SAVED_VIEW";
  private static final String SAVED_ACTION_WITH_ACCOUNT = "com.bai.android.ui.ContactList.SAVED_ACTION_WITH_ACCOUNT";
  private static final String SAVED_ACTION_WITH_GROUP = "com.bai.android.ui.ContactList.SAVED_ACTION_WITH_GROUP";
  private static final String SAVED_ACTION_WITH_USER = "com.bai.android.ui.ContactList.SAVED_ACTION_WITH_USER";
  private static final String SAVED_SEND_TEXT = "com.bai.android.ui.ContactList.SAVED_SEND_TEXT";
  private static final String SAVED_OPEN_DIALOG_USER = "com.bai.android.ui.ContactList.SAVED_OPEN_DIALOG_USER";
  private static final String SAVED_OPEN_DIALOG_TEXT = "com.bai.android.ui.ContactList.SAVED_OPEN_DIALOG_TEXT";

  private static final int OPTION_MENU_SHOW_CONTACTLIST_ID = 0x01;
  private static final int OPTION_MENU_ADD_CONTACT_ID = 0x03;
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    groupChat = (ImageView) findViewById(R.id.group_chat);
    addContact = (ImageView) findViewById(R.id.add_contact);
    startMUC = (TextView) findViewById(R.id.start_muc);
    filterContacts = (EditText) findViewById(R.id.filter_users);
    ...
    startMUC.setOnClickListener(this);
    addContact.setOnClickListener(this);

    filterContacts.addTextChangedListener(this);
    ...
    if (savedInstanceState != null) {
      ...
      isChat = savedInstanceState.getBoolean(SAVED_VIEW);
    } else {
    ...
    if (this.getIntent() != null && this.getIntent().getExtras() != null && this.getIntent().getExtras().getString("isChat") != null) {
      isChat = true;
    } 
  ...
  protected void onSaveInstanceState(Bundle outState) {
    ...
    outState.putBoolean(SAVED_VIEW, isChat);
  }
  ...
  protected void onResume() {
    ...
    //if there are pending chat room invitation - open them	//TODO - do for normal chats as well
    if (MUCManager.getInstance().getPendingIncomingInvitation()) {	//TODO PUT IN OTHER ACTIVITEIS - Bulletin board, AR Camera and Other
      //clear the pending invitations
      MUCManager.getInstance().setPendingIncomingInvitation(false);
      showChat();
    }
    ...
    if (isChat) {
      new TabBar(this, R.id.chat_button); //will call showChat()
    } else {
      new TabBar(this, R.id.friends_button);
    }

    // If application has loaded - redirect if necessary to add accounts view
  ...
  public boolean onCreateOptionsMenu(Menu menu) {
    ...
    menu.add(0, OPTION_MENU_SHOW_CONTACTLIST_ID, 0,
        getText(R.string.show_contact_list)).setIcon(
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    ...
    case OPTION_MENU_SHOW_CONTACTLIST_ID:
      if (showAllContacts == false){
        SettingsManager.setContactsShowOffline(true);
        contactListAdapter.onChange();
        item.setTitle(R.string.show_online_contacts);
        showAllContacts = true;
      } else {
        SettingsManager.setContactsShowOffline(false);
        contactListAdapter.onChange();
        item.setTitle(R.string.show_contact_list);
        showAllContacts = false;
      }
      return true;
  ...
  public void onCreateContextMenu(ContextMenu menu, View view,
    ...
    if (view == getListView()) {
      ...
      BaseEntity baseEntity = (BaseEntity) getListView().getItemAtPosition(info.position);
      if (baseEntity == null)
        // Account toggler
        return;
      if (baseEntity instanceof AbstractContact) {
        ...
        if (MUCManager.getInstance().hasRoom(actionWithAccount, actionWithUser)) {
          if (!MUCManager.getInstance().inUse(actionWithAccount, actionWithUser))
            menu.add(0, CONTEXT_MENU_EDIT_ROOM_ID, 0, getResources().getText(R.string.muc_edit));
          menu.add(0, CONTEXT_MENU_DELETE_CONTACT_ID, 0, getResources().getText(R.string.muc_delete));
          if (MUCManager.getInstance().isDisabled(actionWithAccount,
              actionWithUser))
            menu.add(0, CONTEXT_MENU_JOIN_ROOM_ID, 0, getResources().getText(R.string.muc_join));
          else
            menu.add(0, CONTEXT_MENU_LEAVE_ROOM_ID, 0, getResources().getText(R.string.muc_leave));
        } else {
          menu.add(0, CONTEXT_MENU_VIEW_CONTACT_ID, 0, getResources().getText(R.string.contact_viewer));
          menu.add(0, CONTEXT_MENU_EDIT_CONTACT_ID, 0, getResources().getText(R.string.contact_editor));
          menu.add(0, CONTEXT_MENU_DELETE_CONTACT_ID, 0, getResources().getText(R.string.contact_delete));
          if (MessageManager.getInstance().hasActiveChat(actionWithAccount, actionWithUser))
            menu.add(0, CONTEXT_MENU_CLOSE_CHAT_ID, 0,getResources().getText(R.string.close_chat));
          if (abstractContact.getStatusMode() == StatusMode.unsubscribed)
            menu.add(0, CONTEXT_MENU_REQUEST_SUBSCRIPTION_ID, 0, getText(R.string.request_subscription));
        }
        if (PresenceManager.getInstance().hasSubscriptionRequest(
          ...
          menu.add(0, CONTEXT_MENU_ACCEPT_SUBSCRIPTION_ID, 0,
              getResources().getText(R.string.accept_subscription));
          menu.add(0, CONTEXT_MENU_DISCARD_SUBSCRIPTION_ID, 0, getText(R.string.discard_subscription));
        }
        return;
        ...
        if (baseEntity instanceof AccountConfiguration) {
          ...
          if (actionWithGroup != GroupManager.ACTIVE_CHATS
            ...
            menu.add(0, CONTEXT_MENU_GROUP_RENAME_ID, 0, getText(R.string.group_rename));
            if (actionWithGroup != GroupManager.NO_GROUP)
              menu.add(0, CONTEXT_MENU_GROUP_DELETE_ID, 0, getText(R.string.group_remove));
          }
        }
      } else {
        return;
    ...
    if (actionWithGroup != null || SettingsManager.contactsShowAccounts()) {
      ...
      mapMode.setGroupCheckable(CONTEXT_MENU_SHOW_OFFLINE_GROUP_ID, true, true);
  ...
  public boolean onContextItemSelected(MenuItem item) {
    ...
    case CONTEXT_MENU_VIEW_CHAT_ID:
      MessageManager.getInstance().openChat(actionWithAccount, actionWithUser);
      startActivity(ChatViewer.createIntent(this, actionWithAccount, actionWithUser));
      return true;
    ...
    case CONTEXT_MENU_EDIT_ROOM_ID:
      startActivity(MUCEditor.createIntent(this, actionWithAccount, actionWithUser));
      return true;
    case CONTEXT_MENU_JOIN_ROOM_ID:
      MUCManager.getInstance().joinRoom(actionWithAccount, actionWithUser, true);
      return true;
    case CONTEXT_MENU_LEAVE_ROOM_ID:
      MUCManager.getInstance().leaveRoom(actionWithAccount, actionWithUser);
      MessageManager.getInstance().closeChat(actionWithAccount, actionWithUser);
    ...
    case CONTEXT_MENU_REQUEST_SUBSCRIPTION_ID:
      try {
        PresenceManager.getInstance().requestSubscription(actionWithAccount, actionWithUser);
    ...
    case CONTEXT_MENU_ACCEPT_SUBSCRIPTION_ID:
      try {
        PresenceManager.getInstance().acceptSubscription(actionWithAccount, actionWithUser);
      } catch (NetworkException e) {
        Application.getInstance().onError(e);
      }
      startActivity(ContactEditor.createIntent(this, actionWithAccount, actionWithUser));
    ...
    case CONTEXT_MENU_DISCARD_SUBSCRIPTION_ID:
      try {
        PresenceManager.getInstance().discardSubscription(actionWithAccount, actionWithUser);
      } catch (NetworkException e) {
        Application.getInstance().onError(e);
      }
      return true;
          ...
          this).setMessage(
              getString(
                  ...
                  RosterManager.getInstance().getName(
                      actionWithAccount, actionWithUser))).create();
  ...
  public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
    ...
    if (object == null) {
      ...
      if (ACTION_ROOM_INVITE.equals(action)) {
        ...
        finish();
      } else if (startMUC.getVisibility() == View.VISIBLE) {
        //if currently selecting contacts for multiple user chat, disregards item clicks  
        String user = abstractContact.getUser();
        contactListAdapter.toggleEntitySeleceted(user);
        contactListAdapter.refreshRequest();
        updateMUC_UI();
      } else {
        startActivity(ChatViewer.createIntent(
            this,
            abstractContact.getAccount(),
            abstractContact.getUser()));
      }
  ...
  public static Intent createChatIntent(Context context) {
    Intent chatlistIntent = new Intent(context, ContactList.class);
    Bundle bundle = new Bundle();
    bundle.putString("isChat", "true");
    chatlistIntent.putExtras(bundle);
  ...
  public void showFriends() {
    isChat = false;
    contactListAdapter.showFriends();
    contactListAdapter.refreshRequest();
    filterContacts.setHint(getResources().getString(R.string.search_contact_filter));
  }

  public void showChat() {
    if (startMUC != null && startMUC.getVisibility() == View.VISIBLE) {
      toggleMUCChat();
    }
    isChat = true;
    filterContacts.setHint(getResources().getString(R.string.search_chat_filter));
    contactListAdapter.showChat();
    contactListAdapter.refreshRequest();
  }

  private void toggleMUCChat() {
    contactListAdapter.toggleMucSelector();
    contactListAdapter.refreshRequest();
    if (startMUC.getVisibility() == View.GONE) {
      startMUC.setVisibility(View.VISIBLE);
      startMUC.setTextColor(getResources().getColor(R.color.text_unactive));
    }
    else {
      startMUC.setVisibility(View.GONE);
    }
  }

  private void updateMUC_UI() {
    if (contactListAdapter.getSelectedUsersCount() > 1) {
      startMUC.setTextColor(getResources().getColor(R.color.text_black));
      startMUC.setOnClickListener(this);

    } else {
      startMUC.setTextColor(getResources().getColor(R.color.text_unactive));
      startMUC.setOnClickListener(null);
    }
  }

  private void openGroupChat() {
    // close the user selection view
    toggleMUCChat();
    ...
    String roomName = MUCManager.getInstance().createJoinInviteRoom(
        account,
        contactListAdapter.getSelectedUsers(),
        AccountManager.getInstance().getNickName(account), 
        null);
  ...
  /** Search field change - update view filtering contacts with the given name. */
  @Override
  public void onTextChanged(CharSequence s, int start, int before, int count) {
    contactListAdapter.setFilter(s.toString());
  }

  private String bareAddress, account;
  @Override
  public void onLoad() {
    // automatically redirect to add accounts if there are no accounts regisetered
    if (AccountManager.getInstance().getCommonState() == CommonState.empty) {
      ((Button) findViewById(R.id.button)).performClick();
      return;
    } else {
      //Request to check if the user profile is complete
      account = AccountManager.getInstance().getAccounts().iterator().next();
    ...
    //			Log.e(" Clue. ", " MP ACCPINTS ");
    //			final String account = AccountManager.getInstance().getAccounts().iterator().next();
    //			AccountItem accountItem = AccountManager.getInstance().getAccount(account);
    //			ConnectionState state = accountItem.getState();
    //			if (state.isConnected()) {
    //				Log.e(" Clue. ", " CREATE PERSISTANT CHATS ");
    //				MUCManager.getInstance().preparePersistentChats();
    //			}
    //		}
  }
  ...
  public void onVCardReceived(String account, String bareAddress, VCard vCard) {
    ...
    if (vCard.getEmail() == null || vCard.getEmail().length() == 0) {
      Toast.makeText(getApplicationContext(), getResources().getString(R.string.incomplete_profile), 1).show();
      Toast.makeText(getApplicationContext(), getResources().getString(R.string.please_complete_profile), 1).show();
      Intent addAccount = new Intent(getApplicationContext(), OtherActivity.class);
