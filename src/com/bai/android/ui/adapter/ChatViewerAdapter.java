...
import com.bai.android.data.message.chat.ChatManager;
...
import com.bai.android.ui.helper.ContactTitleInflater;
...
public class ChatViewerAdapter extends BaseAdapter implements SaveStateAdapter,
	...
	public ChatViewerAdapter(Activity activity, String account, String user) {
		...
		Collection<? extends BaseEntity> activeChats = MessageManager.getInstance().getPersonalActiveChats();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			ChatMessageAdapter chatMessageAdapter = new ChatMessageAdapter(activity, false);
			...
			chatViewHolder.list.setDividerHeight(0); //no divider
			chatViewHolder.list.setDivider(null); //no divider
			...
			chatViewHolder.input.addTextChangedListener(new TextWatcher() {
				...
				public void afterTextChanged(Editable s) {
					...
					if (onTextChangedListener != null)
						onTextChangedListener.onTextChanged(chatViewHolder.input, s);
				}
		...
		if (chat.equals(chatViewHolder.chatMessageAdapter.getAccount(), chatViewHolder.chatMessageAdapter.getUser())) {
			...
			chatViewHolder.input.setSelection(
		...
		chatViewHolder.page.setText(activity.getString(R.string.chat_page, position + 1, getCount()));
		ContactTitleInflater.updateTitle(chatViewHolder.title, activity, abstractContact);
		avatarInflaterHelper.updateAvatar(chatViewHolder.avatar, abstractContact);
		...
		if (securityLevel == SecurityLevel.plain
				&& (securityOtrMode == SecurityOtrMode.disabled
				|| securityOtrMode == SecurityOtrMode.manual)) {
...
//		Toast.makeText(activity.getApplicationContext(), "ON CHAT CHANGE?", 0).show();
	...
	public void onChange() {
		activeChats = new ArrayList<AbstractChat>(MessageManager.getInstance().getActiveChats());
		if (intentPosition != -1) {
