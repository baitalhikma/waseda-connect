...
import com.bai.android.data.message.ChatContact;
		...
		if (filterString == null) {
			...
			for (RosterContact rosterContact : rosterContacts) {
				...
				if (showActiveChats && abstractChat != null && abstractChat.isActive()) {
					...
					//					if (!stayActiveChats || (!showAccounts && !showGroups)) continue;
					if (!stayActiveChats || (!showGroups)) continue;
				}
				...
				if (addContact(rosterContact, online, accounts, groups,
						contacts, showGroups, showOffline))
					hasVisible = true;
				...
				for (TreeMap<String, AbstractChat> users : abstractChats.values())
					for (AbstractChat abstractChat : users.values()) {
						...
						if (abstractChat instanceof RoomChat) {
							//Don't include the global chats in the list
							if (abstractChat.getUser().equalsIgnoreCase(MUCManager.PersistentMUCChats.GLOBAL) ||
									abstractChat.getUser().equalsIgnoreCase(MUCManager.PersistentMUCChats.NOTICE) ||
									abstractChat.getUser().equalsIgnoreCase(MUCManager.PersistentMUCChats.RECOMMENDED)) continue;
						...
						if (abstractChat instanceof RoomChat) {
							if (!showActiveChats) continue; //don't show room chats in friends list
...
//					Collections.sort(contacts, comparator);	TODO TEST FRIENDS-CHAT/ONLINE - OFFLINE/SEND MESSAGES
//					baseEntities.addAll(contacts);
			...
			if (!showActiveChats) {
				...
				for (RosterContact rosterContact : rosterContacts) {
					...
					if (rosterContact.getName().toLowerCase(locale).contains(filterString)) { //TODO discrepency - display name and user's account name
				...
				for (TreeMap<String, AbstractChat> users : abstractChats.values())
					for (AbstractChat abstractChat : users.values()) {
						...
						// show chats, excluding global chats
						if (abstractContact.getName().toLowerCase(locale).contains(filterString) && 
								!abstractContact.getName().toLowerCase(locale).contains(MUCManager.PersistentMUCChats.GLOBAL) &&
								!abstractContact.getName().toLowerCase(locale).contains(MUCManager.PersistentMUCChats.RECOMMENDED) && 
								!abstractContact.getName().toLowerCase(locale).contains(MUCManager.PersistentMUCChats.NOTICE)) {
		...
		synchronized (refreshLock) {
			...
			if (refreshRequested) handler.postDelayed(this, REFRESH_INTERVAL);
		}
	}
