...
//	private boolean isMUC;
	...
	private long MILS_IN_A_DAY = 86400000;
...
//	private final int appearanceStyle;
	 ...
	 */
	public ChatMessageAdapter(Activity activity, boolean globalMUCFlag) {
...
//		if (convertView == null) {
			...
			if (type == TYPE_MESSAGE) {
				if (!noticeView) {
					...
					layout_resource = R.layout.chat_viewer_message_notice;
				}
			}
...
//		textView.setTextAppearance(activity, appearanceStyle);
		...
		long messageTime = messageItem.getTimestamp().getTime();
		
		String time = null;
		if (now - messageTime < MILS_IN_A_DAY) { //if the message is older than one day - show the date
			time = StringUtils.getSmartTimeText(messageItem.getTimestamp());
		} else {
			time = messageItem.getTimestamp().getDay() + "." + messageItem.getTimestamp().getMonth()+ " " + StringUtils.getSmartTimeText(messageItem.getTimestamp());
		}
...
//			append(builder, " ", new ImageSpan(activity, messageResource));
			...
			append(builder, " ", new TextAppearanceSpan(activity, R.style.ChatHeader));
			// in notice view don't show additional information
			if (!noticeView) {
...
//				Date timeStamp = messageItem.getDelayTimestamp();   DELAY TIME
	//				if (timeStamp != null) {
	//					
...
//				}
	...
	private boolean noticeView = false;
