...
public abstract class BaseContactAdapter<Inflater extends BaseContactInflater>
	...
	public void toggleEntitySeleceted(String user) {
		for (BaseEntity baseEntity : baseEntities) {
			if (baseEntity.getUser() == user) {
				baseEntity.toggleSelected();
				break;
			}
		}
	}

	public ArrayList<BaseEntity> getSelectedUsers() {
		ArrayList<BaseEntity> selectedEntitites = new ArrayList<BaseEntity>();
		for (BaseEntity baseEntity : baseEntities) {
			if (baseEntity.isSelected()) {
				selectedEntitites.add(new BaseEntity(baseEntity));
			}
		}
		return selectedEntitites;
	}

	public int getSelectedUsersCount() {
		int selectedEntititesCount = 0;
		for (BaseEntity baseEntity : baseEntities) {
			if (baseEntity.isSelected()) {
				selectedEntititesCount++;
			}
		}
		return selectedEntititesCount;
	}
	...
	/** Apply a name filter on the displayed contacts. */
